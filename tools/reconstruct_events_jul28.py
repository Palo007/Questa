...
# 2026-09-19: these two anchors are PLACEHOLDERS. They used to be two task ids
# and two habit titles from the maintainer's own device. Not a secrecy matter --
# REVIEW-2026-09-18.md Part 3 called them 'real personal data', which overstated
# it; they were opaque UUIDs and two pushup counts. The reason they are gone is
# that a script meant to be readable by anyone should not hardcode ONE person's
# task ids and then assert they exist.
#
# That assertion is below, so running this against your own backup will fail here
# until you replace both entries with two task ids that genuinely exist in YOUR
# snapshot. The failure is intended: the assertion proves the detokenization
# resolved real rows, and it cannot do that against invented ids.
ANCHOR_IDS = {
