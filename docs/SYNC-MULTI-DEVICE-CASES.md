...
| 4.26 | Merge skew clamp bypassed at 8 sites (P10a) — **CLOSED by F1 (`f952f7a`, 2026-08-19)**, re-verified K5 2026-09-11 | Clamped, by symbol: `resolveDailyConflict` `sync.js:590`; `mergeChecklist` text/done tiebreaks `sync.js:680`/`:691` (operands minted `:669`); `mergeCollection` both-changed `sync.js:958`; char F6 tiebreak `sync.js:1471`. Raw **by design**: `mergeChecklist`'s `Math.max(lt, rt)` `sync.js:693` (failure mode at `:663-668`) and the survivor-vs-deletion test `sync.js:707-711` ("Site 1 of 4") | Device stuck in future wins every conflict | K5 D13: **zero accidental bypasses remain.** All five `_clampFuture` call sites were introduced by `f952f7a` per `git log -S`; `_uaRaw` (K1) came later in `2e346ac`, so **K1 is orthogonal** — it hardened the deliberately-raw sites, it did not add clamps. The two raw sites are the inverted-polarity class governed by D2/D5, **not** bypasses. Do not re-audit the 8. Evidence: `.omo/evidence/K5-sync-backlog/cycle-1-findings.md` §1 | **CLOSED** |
...
| 4.28 | Char merges as whole-object LWW (P10c) — **FIXED by K3 (2026-09-11)** | `sync.js:1209` (`_charAccumulate`), applied at `sync.js:1414` (`char:`) | Two devices earning xp/gold offline lost one side's rewards | Total xp / gold / mp now accumulate three-way on top of the LWW winner; `lvl`+`xp` re-derived from the total; idempotent across the conflict-retry path via `char.abs` (inverted-polarity site 7 of 7). Stands down to the old LWW when base is missing/empty, a side is non-canonical, or a device id is absent. Habit `cUp`/`cDown` accumulate too (`sync.js:1271`). `hp`/`maxHp`/`name`/`face`/`cls`/`streak` deliberately stay LWW. Design + rejected event-log replay: `.omo/plans/K3-earnings-design.md`. Tests: `tests/earnings-accumulate.test.js` | **FIXED** |
...
| 4.33 | Char deviceId tiebreak fired when it must not (K4) — **FIXED 2026-09-11** | `sync.js:1471` (`_lUa`/`_rUa` hoist), `sync.js:1489` (gated tiebreak) | The both-changed char arm tested only "remote is strictly newer" and then fell straight into the deviceId tiebreak, so the tiebreak also fired when **local** was strictly newer. Each device adopted the other's `name`/`face`/`cls`/`hp`/`maxHp`/`id`/`updatedAt` and the pair swapped them on every sync round, forever. `hp` is the non-cosmetic one: a damaged device kept having its HP restored by a peer that took no damage. Post-K3 this was scalar-only — `xp`/`gold`/`mp` already converged via `_charAccumulate`. | Gated on an exact clamped tie (`_lUa === _rUa`), matching the three other tiebreak sites (`resolveDailyConflict` `sync.js:596`, `mergeCollection` non-daily arm `sync.js:962`, `mergeDevices` `sync.js:1109`). Local-strictly-newer now falls through to the local return, the exact mirror of the strictly-newer-remote branch. Both operands are clamped **once** and reused, because `_futureCeil()` is `Date.now()`-derived and two calls can disagree across a tick. No new clamp introduced (both were already clamped by F1). The conflict-log `reason` at the fall-through is now conditional, since two paths reach it. All four char guards unchanged: `untouched()`, the died-character carve-out, the deliberately unclamped one-sided branch, and the tiebreak itself. Tests: `tests/char-tiebreak-convergence.test.js` | **FIXED** |
| 4.34 | Cross-timezone daily echo un-completed a still-current daily (K5) — **FIXED 2026-09-11** | `_isCronEchoReset()` `sync.js:622-646`; GUARD 2 at `sync.js:946-953`, inside `mergeCollection`'s one-sided branch | `normalizeDailyResets` builds `Object.assign({}, t, {done:false})`: it **keeps `doneAt`** and **never bumps `updatedAt`**. A device in a later timezone crossed local midnight first, reset the daily, and uploaded a record differing from base by the `done` flag alone. On a device whose own day had **not** rolled over, `localChanged` was false and `remoteChanged` true, so the one-sided branch fired; GUARD 1 only rescues local when `_uaRaw(l) > _uaRaw(r)`, and the echo carries the **same** `updatedAt`, so it slipped through. A completion the user really made today was erased, along with every subtask tick, and the XP/gold already granted was not re-granted. K1 fixed the sibling case (a remote `lastCron` in this device's future) in `mergedLastCron`; the echo arrives on the record itself and never reaches that rule. | GUARD 2 keeps `l` **wholesale** when all of: either side is a `daily`; `l.done === true && r.done === false`; `_uaRaw(l) === _uaRaw(r)`; `r.doneAt` is present; and `dayStampOf(r.doneAt) >= dayStampOf(Date.now())`. A deliberate un-tick is a different shape — `uncompleteDaily`/`uncompleteTodo` (`app.js:1818-1821`, `1829-1831`) **both** `delete t.doneAt` **and** bump `updatedAt` — so a real retraction still wins. **No F4 checklist splice** on this path: `normalizeDailyResets` clears the entire checklist when it resets, so the remote side is all-false by construction and splicing it would re-clear the ticks the guard exists to save. Inverted polarity — a false destroys local data — so every operand reads RAW and the day test is `>=`; **no new clamp site**, D5 untouched. Convergence holds: the device that *has* crossed midnight still resets (K5-F). No schema change; D11 re-run only. Tests: `tests/daily-crosstz-echo-k5.test.js` (K5-A/B `[K5-NEW]`, K5-C/D/E/F `[GUARD]`) | **FIXED** |
...
| 6.6 | Merge-engine fixes: ~~skew clamp at all sites~~, subtask tombstones, ~~char per-field merge~~ — **char per-field merge SHIPPED in K3 (2026-09-11)**, see row 4.28; **skew clamp CLOSED by F1 (`f952f7a`, 2026-08-19)**, see row 4.26 and K5 D13. Still open here: subtask tombstones, and a three-way `charHistory` (K3 changed only its day bucket from UTC to local — `Math.max` per key means a decrease, e.g. spent gold or a death loss, still cannot propagate into the history series) | `.omo/plans/merge-hardening-v1.md`, `.omo/plans/K3-earnings-design.md` |
