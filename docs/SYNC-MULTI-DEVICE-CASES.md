...
| 6.6 | Merge-engine fixes: skew clamp at all sites, subtask tombstones, ~~char per-field merge~~ — **char per-field merge SHIPPED in K3 (2026-09-11)**, see row 4.28. Still open here: subtask tombstones, and a three-way `charHistory` (K3 changed only its day bucket from UTC to local — `Math.max` per key means a decrease, e.g. spent gold or a death loss, still cannot propagate into the history series) | `.omo/plans/merge-hardening-v1.md`, `.omo/plans/K3-earnings-design.md` |
