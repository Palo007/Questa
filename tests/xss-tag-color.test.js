...
//   COL4b tagFilterBar, COL4c/COL4d tagEditorBlock (own / pick chips): --tc is the fallback
...
const jsqSrc = extractFunction(src, /^function jsq\(/, 'jsq');
const barSrc = extractFunction(src, /^function tagFilterBar\(/, 'tagFilterBar');
const edSrc = extractFunction(src, /^function tagEditorBlock\(/, 'tagEditorBlock');
...
const api = new Function(
  ...
  'var FILTEROPEN = true, TAGFILTER = {};\n' +
  escSrc + '\n' + cssSrc + '\n' + listSrc + '\n' + legSrc + '\n' + sumSrc + '\n' + chipSrc + '\n' +
  jsqSrc + '\n' + barSrc + '\n' + edSrc + '\n' +
  tagColorsLine + '\n' +
  'return { cssColor, anListHTML, anRowLegend, anTagSummaryBody, tagChips, tagFilterBar, tagEditorBlock, TAG_COLORS };'
)();
...
HOSTILE.forEach((c, i) => {
  ...
  // COL4b/COL4c: every --tc value must be exactly the fallback (the None button
  // already uses var(--muted), so a raw hostile value shows up as a mismatch).
  const tcs = h => (h.match(/--tc:[^"]*/g) || []);
  const onlyMuted = h => { const v = tcs(h); return v.length > 0 && v.every(x => x === '--tc:var(--muted)'); };
  S.tags = [{ id: 't1', name: 'W', color: c }, { id: 't2', name: 'X', color: c }];
  const fb = api.tagFilterBar('habits');
  assert('COL4b.' + i + ' tagFilterBar tag button --tc carries only the fallback',
    clean(fb) && tcs(fb).length === 3 && onlyMuted(fb));
  const ed = api.tagEditorBlock({ tags: ['t1'] });
  const cut = ed.indexOf('tagPick');
  const own = ed.slice(0, cut), pick = ed.slice(cut);
  assert('COL4c.' + i + ' tagEditorBlock own chip --tc carries only the fallback', cut > 0 && clean(own) && onlyMuted(own));
  assert('COL4d.' + i + ' tagEditorBlock pick chip --tc carries only the fallback', cut > 0 && clean(pick) && onlyMuted(pick));
});
